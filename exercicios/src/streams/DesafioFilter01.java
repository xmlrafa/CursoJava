...
public class DesafioFilter01 {
	public static void main(String[] args) {
		...
		Function<Encomendas, String>Resultado= r->"O(s) pedido(s): "+ r.nomeProduto + " de "+r.nomeCliente+ " Está(ão)"+r.statusEncomenda;
...
//		
		encomendas.stream().filter(aCaminho).filter(s -> s.nomeProduto=="Blusa").map(Resultado).forEach(System.out::println);
	}
}
